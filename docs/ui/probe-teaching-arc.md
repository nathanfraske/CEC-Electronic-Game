...
1. **The cold-open is failure-first, and it supersedes the working-primer cold open** for the scaffolded path. Today `onMount` auto-loads `'primer'` (a healthy V→R loop) **PAUSED** [App.svelte ~2098]. This panel replaces that *as the front door of the pulled/scaffolded arc* with a **new resistor-less `'led-blowup'` ExampleSpec, also loaded PAUSED** — and (see the load-bearing expert decision below) **fired only on the player's own Run**, never auto-run. This **refines, does not contradict, [onboarding-first-run.md] §1's** "auto-play-then-pause" beat: the spectacle is armed-but-paused, so a competent user who starts building never sees it. It is the diegetic embodiment of **[game-progression.md] §1's** "the bench dares you."
...
- **Speech:** real **DOM text** in the established coach-mark register (Saira body, IBM Plex Mono for any telemetry), plus, for pre-readers, **picture speech-bubbles** (an LED+smiley glyph, a "speed-bump" resistor glyph, a target-bar glyph) and an **optional spoken voice layer** — all **captioned in real DOM text** (`aria-live` throttled ~2 Hz, screen-reader-legible, translatable). Meaning never lives in his words, colour, or motion alone.
...
1. **Cold-open auto-run vs fire-on-Run.** [onboarding §1] specifies auto-play-then-pause for the *healthy* primer. This panel **strongly recommends the resistor-less cold-open load PAUSED and fire the smoke only on the player's own Run** (the load-bearing expert-safety choice). **Owner must bless this refinement of §1.** *Stated fallback if the owner keeps motion-first:* auto-play a **healthy** `led-limit` for ~4 s (motion-first hook, no failure), then the Probe **rebuilds it broken** and invites the player to press play — preserving BOTH the §1 auto-play hook AND the self-caused-but-blameless blow-up.
...
11. **Voice production:** TTS vs recorded VO for the Probe (warmth/bonding for young kids vs authoring/localization cost). **Owner call on budget + the persona's "sound."**
