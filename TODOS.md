...
- [ ] **Wire the diode into the web** (now solver-ready): `netlist.ts` TYPE_OF `D:5`, an animated `drawD` glyph (arrow + bar, conduction glow), mark `D` ideal/placeable in the bin. Then LED (diode + light) and a diode example (clipper/rectifier).
- [ ] **1-pin part hit box** (GND): the bounding box is too small to grab — widen `componentBox`/pin hit area for 1-pin parts so GND is easy to click + drag.
- [ ] **Dangling-source affordance**: a V source needs a closed loop, but an I source with a floating terminal still "solves" (confusing). Detect a source pin with no return path and flag it (ghost the part / hint) rather than silently simulating.
