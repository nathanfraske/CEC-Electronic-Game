...
export const EXAMPLES: ExampleSpec[] = [
  ...
  {
    id: "counter-ramp",
    name: "Counter → DAC Ramp Generator",
    blurb:
      "Make a waveform out of pure logic. A clock drives a 3-bit binary counter, which steps through 0, 1, 2 … 7 and rolls over; feed that count straight into a DAC and the output climbs one step per clock and snaps back at the top — a self-running staircase ramp (a sawtooth). No analog oscillator anywhere: the shape is generated by counting. It is exactly the flash-ADC staircase from the last example, but the code now comes from a counter instead of a measured input.",
    watch:
      "widen the scope's time-base a notch and watch AOUT climb in eight even 0.625 V steps from 0 to 4.375 V, then drop straight back to 0 and repeat — a sawtooth built one count at a time. Each clock edge bumps the count by one (Q0 toggles every edge, Q1 every two, Q2 every four), and the DAC turns that binary count into the voltage step.",
    build() {
      // A square-wave clock (PULSE) → 3-bit counter → R-2R DAC → AOUT. The counter free-runs
      // (RESET unwired), stepping the DAC code 0..7 and wrapping, so AOUT is a sawtooth staircase.
      // Counter pins: CLK(0) RESET(1) Q2(2) Q1(3) Q0(4) VCC(5) GND(6).
      // R-2R DAC pins: AOUT(0) GND(1) D0(2) D1(3) D2(4) VCC(5).
      const g = new BoardGraph();
      const clk = comp(g, "PULSE", 0, 1, 2000); // 2 kHz square clock (one 8-count ramp ≈ 4 ms)
      clk.amp = 5; // 0..5 V clock so it crosses the counter's logic threshold
      clk.variant = 0; // square wave (the clock; vs the triangle of the ADC demo)
      clk.duty = 0.5;
      const ctr = comp(g, "CTR", 5, 0, 7);
      const dac = comp(g, "DAC", 11, 0, 0);
      const vcc = comp(g, "V", 0, 8, 5, 1); // 5 V supply (vertical), + at top
      const gnd = comp(g, "GND", 8, 8, 0);
      wire(g, clk, 0, ctr, 0); // clock → CTR.CLK
      wire(g, clk, 1, gnd, 0); // clock − → GND
      wire(g, vcc, 0, ctr, 5); // +5 V → CTR.VCC
      wire(g, ctr, 6, gnd, 0); // CTR.GND → GND
      wire(g, vcc, 0, dac, 5); // +5 V → DAC.VCC (full-scale reference)
      wire(g, dac, 1, gnd, 0); // DAC.GND → GND
      wire(g, ctr, 4, dac, 2); // CTR.Q0 → DAC.D0 (LSB)
      wire(g, ctr, 3, dac, 3); // CTR.Q1 → DAC.D1
      wire(g, ctr, 2, dac, 4); // CTR.Q2 → DAC.D2 (MSB)
      wire(g, vcc, 1, gnd, 0); // V− → GND (the 0 V reference)
      g.addNetLabel({ componentId: clk.id, pinIndex: 0 }, "CLK");
      g.addNetLabel({ componentId: dac.id, pinIndex: 0 }, "AOUT");
      g.addNetLabel({ componentId: vcc.id, pinIndex: 0 }, "+5V");
      g.addNetLabel({ componentId: gnd.id, pinIndex: 0 }, "GND");
      return g.serialize();
    ...
    steps: [
      {
        do: "Place a Pulse source (PULSE, square wave) and a Counter (CTR). Power the counter (VCC to +5 V, GND to ground) and wire the pulse to its CLK. Run.",
        why: "The counter adds one on every rising clock edge, marching its outputs Q0/Q1/Q2 through the binary numbers 0..7 and rolling over. Scope the three Q pins and watch them count — the lowest bit toggles fastest, each higher bit at half the rate.",
        done: (p) => at(p, "PULSE") >= 1 && at(p, "CTR") >= 1 && p.wires >= 3,
      },
      {
        do: "Place an R-2R DAC. Wire the counter's Q0/Q1/Q2 to the DAC's D0/D1/D2, power the DAC (VCC to +5 V, GND to ground), and tap AOUT. Run.",
        why: "The DAC turns each count into a voltage (AOUT = count/8 · 5 V). Now the counting becomes a staircase: AOUT climbs one step per clock and snaps back at 7 — a sawtooth, synthesised purely by counting.",
        done: (p) => at(p, "CTR") >= 1 && at(p, "DAC") >= 1 && p.complete,
      },
      {
        do: "Open the scope, widen the time-base, and watch AOUT.",
        why: "A clean 8-step ramp that repeats — a digital waveform generator. Speed it up or down by changing the clock; widen the counter and the DAC and the steps get finer. This same counter-plus-DAC idea drives function generators, and the counter alone is the heart of timers, clock dividers, and sequencers.",
        done: (p) => p.complete,
      },
    ],
  },
